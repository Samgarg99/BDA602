...
def Baseball():
    df = pd.read_table("./stuff/combined5_1.csv")

    print("\n\n\n\n testing ", df.head(), df.columns)
    ...
    df["winner_home_or_away"] = df["winner_home_or_away"].replace("H", 1)
    df["winner_home_or_away"] = df["winner_home_or_away"].replace("A", 0)
...
def cat_resp_cont_predictor(feature_name, x, y, response="target"):
    ...
    # fig.show()
    Path("./stuff/plots/").mkdir(parents=True, exist_ok=True)
    fig.write_html(f"./stuff/plots/{feature_name} and {response}.html")
    url = f"./plots/{feature_name} and {response}.html"
    return url
...
def cat_resp_cat_predictor(x, y, i, response="target"):
    ...
    # fig.show()
    Path("./stuff/plots/").mkdir(parents=True, exist_ok=True)
    fig.write_html(f"./stuff/plots/{i} and {response}.html")
    url = f"./plots/{i} and {response}.html"
...
def MeanOfResponseCorr(means_new, col1, col2):
    ...
    Path("./stuff/plots/").mkdir(parents=True, exist_ok=True)
    fig.write_html(f"./stuff/plots/mor {col1} and {col2}.html")
    url = f"./plots/mor {col1} and {col2}.html"
    return url
...
def MeanOfResponse(
    ...
    if X_type == "Continuous" and X2_type == "Continuous":
        cuts = pd.DataFrame(
            {
                str(col1) + " Bin": pd.cut(temp_df[col1], ncut1),
                str(col2) + " Bin": pd.cut(temp_df[col2], ncut2),
            }
        )
    elif X_type == "Continuous" and X2_type != "Continuous":
        cuts = pd.DataFrame(
            {
                str(col1) + " Bin": pd.cut(temp_df[col1], ncut1),
...
def PTScore(x, y, res_type, feature_name):
    ...
    Path("./stuff/plots/").mkdir(parents=True, exist_ok=True)
    fig.write_html(f"./stuff/plots/{feature_name}_ptscore.html")
    url = f"./plots/{feature_name}_ptscore.html"
...
def MeanOfResponseCont(X, y, col, target, restype="Continuous", bin_size=10):
    ...
    fig.update_yaxes(title_text="Response Mean", secondary_y=False)

    # fig.show()
    Path("./stuff/plots/").mkdir(parents=True, exist_ok=True)
    fig.write_html(f"./stuff/plots/{col}_mor.html")
    url = f"./plots/{col}_mor.html"
...
def html(df0, df1, df2, df3, df4, df5, df6, link1, link2, link3):
    ...
    # OUTPUT AN HTML FILE
    with open("./stuff/myhtml_3.html", "w") as f:
        f.write(
            html_string.format(
        ...
        for j in cont_features:
            if i != j and (i, j) not in cont_list and (j, i) not in cont_list:
                url, w_score, uw_score = MeanOfResponse(
                    X[i], X[j], y, i, j, target, restype=target_type
                )
...
def ml_model(df, predictors, target):
    ...
    clf = make_pipeline(
        StandardScaler(), DecisionTreeClassifier(criterion="entropy", max_depth=3)
    )
    clf.fit(X_train, y_train)
    y_pred = clf.predict(X_test)
